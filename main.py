...
# Please refer http://philipperemy.github.io/information-extract/
...
tmp_folder = '/Users/ruizhang/Documents/Finance_relationship_extractor/out'
...
def generate_graphviz_graph(entity_relations, verbose=True):
    ...
    out_dot = os.path.join(tmp_folder, 'out.dot')
    with open(out_dot, 'w') as output_file:
        output_file.writelines(graph)

    out_png = os.path.join(tmp_folder, 'out.png')
...
def stanford_ie(input_filename, verbose=True, generate_graphviz=False):
    out = os.path.join(tmp_folder, 'out.txt')
